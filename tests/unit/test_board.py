...
def test_read_board_not_found(test_client: TestClient, monkeypatch: MonkeyPatch):
    ...
    assert response.json()["detail"] == "Board with id=1 not found"
...
def test_create_board_already_exists(test_client: TestClient, monkeypatch: MonkeyPatch):
    test_request_data = {
        "name": "board1"
    }
...
def test_delete_board(test_client: TestClient, monkeypatch: MonkeyPatch):
    ...
    def mock_delete_board(db: Session, board: schemas.Board):
        return test_data

    monkeypatch.setattr(crud, "get_board_by_id", mock_get_board_by_id)
    monkeypatch.setattr(crud, "delete_board", mock_delete_board)
