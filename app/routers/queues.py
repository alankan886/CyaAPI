...
async def create_queue(queue: schemas.QueueCreate, db: Session = Depends(get_db)):
    if crud.read_queue_by_name(queue):
        raise HTTPException(
            status_code=400,
...
async def update_queue(queue_id: str, new_info: schemas.QueueUpdate, db: Session = Depends(get_db)):
    db_queue = crud.read_queue_by_id(db, queue_id)
    if crud.read_queue_by_id(queue_id):
        raise HTTPException(
...
async def remove_queue(queue_id: str, db: Session = Depends(get_db)):
    db_queue = crud.read_queue_by_id(db, queue_id)
    if not db_queue:
        raise HTTPException(status_code=404, detail=f"Queue with id='{db_queue}' is not found")
